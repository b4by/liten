...
function displayPoints() {
  if (nextPoint >= points.length) return;
...
function displayElastic() {
  if (nextPoint <= 0 || nextPoint >= points.length) return;
...
function getNext() {
  if (nextPoint < 0 || nextPoint >= points.length) return null;
...
function displayInstructions() {
  ...
  if (nextPoint == 0) {
    text(
      "Use your mouse to connect the dots in the specified order.",
      width / 2,
      height - 10
    );
